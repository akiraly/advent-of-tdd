...
class Day09p2Tests : FunSpec({
  ...
  context("Predicting previous value") {
    table(
      headers("Series", "Previous value"),
      row(listOf(0L, 3L, 6L, 9L, 12L, 15L), -3L),
      row(listOf(1L, 3L, 6L, 10L, 15L, 21L), 0L),
      row(listOf(10L, 13L, 16L, 21L, 30L, 45L), 5L)
    ).forAll { series, prediction ->
      test(""" Given "$series" then the previous value should be "$prediction" """) {
        series.predictPreviousValue() shouldBe prediction
      }
    }
  }

  test("Given $exampleOASISReport then the sum of all predicted previous values should be 2") {
    exampleOASISReport.toOASISReport().sumOfPredictedPreviousValues() shouldBe 2
  }

  test("Given our custom OASIS report then the sum of all predicted previous values should be 993") {
    readInput("day09p1").toOASISReport().sumOfPredictedPreviousValues() shouldBe 993
  }
})
