...
class Day24p2Tests : FunSpec({
  val exampleInput = """
    ...
    20, 19, 15 @  1, -5, -3
  """.trimIndent()
  val exampleTestRange = 7.toBD()..27.toBD()

  val customInput = readInput("day24p1")
  val customTestRange = 200_000_000_000_000.toBD()..400_000_000_000_000.toBD()
  ...
  context("test 2d intersection") {
    table(
      ...
      test("""Given the hailstone A $hailstoneA and hailstone B $hailstoneB then the intersection should be $expectedIntersection""") {
        ...
        if (expectedIntersection != null) {
          ...
          intersection.eq(Vector2D.parse(expectedIntersection)) shouldBe true
          intersection.eq(reversed) shouldBe true
        } else {
      ...
      test("""Given the hailstone A $hailstoneA and hailstone B $hailstoneB then the intersection in test range $exampleTestRange should be $expectedIntersectionInTestRange""") {
        ...
        if (expectedIntersectionInTestRange != null) {
          requireNotNull(intersection)
          requireNotNull(reversed)
          intersection.eq(Vector2D.parse(expectedIntersectionInTestRange)) shouldBe true
          intersection.eq(reversed) shouldBe true
        } else {
          intersection shouldBe null
          reversed shouldBe null
    ...
    test(""" Given the custom hailstone list then the collision stone should be (344525619959965, 437880958119624, 242720827369528, -99, -269, 81) with px + py + pz = 1025127405449117""") {
      val hailstones = customInput.toHailstones()

      val collisionStone = calcCollisionStone(hailstones)
      collisionStone shouldBe Hailstone(
        px = 344525619959965,
        py = 437880958119624,
        pz = 242720827369528,
        vx = -99,
      ...
      with(collisionStone) {
        px + py + pz shouldBe 1025127405449117
      }
    }
  }
})
