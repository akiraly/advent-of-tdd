...
class Day07p1Tests : FunSpec({
  ...
  context("Test Hands") {
    table(
      ...
      row("T55J5", listOf(Ten, Five, Five, Jack, Five)),
      row("KK677", listOf(King, King, Six, Seven, Seven)),
      row("KTJJT", listOf(King, Ten, Jack, Jack, Ten)),
      row("QQQJA", listOf(Queen, Queen, Queen, Jack, Ace)),
  ...
  context("It should be possible to calc the type of a Hand") {
    table(
      ...
      row("T32KQ", HighCard),
      row("32T3K", OnePair),
      row("T55J5", ThreeOfAKind),
      row("KK677", TwoPair),
      row("KTJJT", TwoPair),
      row("QQQJA", ThreeOfAKind),
...
QQQJA 483
  ...
  test("The example input can be parsed into a list of HandBids") {
    exampleInput.toHandBidList() shouldBe listOf(
      ...
      HandBid(Hand(listOf(Ten, Five, Five, Jack, Five)), 684),
      HandBid(Hand(listOf(King, King, Six, Seven, Seven)), 28),
      HandBid(Hand(listOf(King, Ten, Jack, Jack, Ten)), 220),
      HandBid(Hand(listOf(Queen, Queen, Queen, Jack, Ace)), 483)
    )
  }

  test("Given the example input then we can calculate the total winning as 6440") {
    exampleInput.toHandBidList().totalWinning() shouldBe 6440
  }

  test("Given our custom input then we can calculate the total winning as 246424613") {
    readInput("day07p1").toHandBidList().totalWinning() shouldBe 246424613
  }
})
